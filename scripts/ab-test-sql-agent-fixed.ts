/**
 * SQL Query Agent A/B Testing Suite (Fixed Version)
 *
 * This script runs comparison tests between legacy and enhanced SQL agents
 * Simplified version without complex ANSI color codes that cause esbuild issues.
 *
...
function getSymbol(success: boolean): string {
...
function logResult(result: TestResult) {
  ...
  const icon = hasExpectedColumns && hasExpectedFilters && hasExpectedName && returnedData
    ? '[GOOD]' : '[WARN]';
  const color = success ? '[SUCCESS]' : '[WARN]';
